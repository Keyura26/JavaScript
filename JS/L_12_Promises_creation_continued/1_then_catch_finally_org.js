...
//*******All the multiple then will be executed if its resolved or else moves to catch directly**********/
 // If either of then fails catch will catch it
//these chain are serial not synchronous
//when we chain we say that all three tasks are dependent if either of them fails we catch it
//as on the whole it fails
 ...
 * Insight : catch is subset of then
*/
 ...
 * 3. for chained then if then above you  returns a value  or either of above 2 points the below then gets a resolved promise
 ...
 * (These are the three ways in which we can reject a promise from then) fetch().then().catch()
 * 
...
/**
 *
 * 3. finally returns a value / promise that will be resolved -> then its return  value is ignored &
...
/**
* 4. finally recives  a reject/ throw new Error it executes itself and if it itself not returning (any error or rejected promise)
